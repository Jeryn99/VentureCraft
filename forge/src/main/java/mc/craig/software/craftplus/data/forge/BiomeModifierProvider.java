...
public record BiomeModifierProvider(DataGenerator dataGenerator) implements DataProvider {
    ...
    public static void generate(RegistryOps<JsonElement> ops, BiomeModifier modifier, Path outputFolder, String saveName, CachedOutput cache) {
        ...
        final String biomeModifierPathString = String.join("/", directory, VentureCraft.MODID, biomeModifiersRegistryID.getNamespace(), biomeModifiersRegistryID.getPath(), saveName + ".json");

        BiomeModifier.DIRECT_CODEC.encodeStart(ops, modifier).resultOrPartial(msg -> VentureCraft.LOGGER.error("Failed to encode {}: {}", biomeModifierPathString, msg)).ifPresent(json ->
        {
            try {
    ...
    public @NotNull String getName() {
        return VentureCraft.MODID + " Biome Modifiers";
    }
}