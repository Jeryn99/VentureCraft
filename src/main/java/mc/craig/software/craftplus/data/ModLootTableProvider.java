...
public class ModLootTableProvider extends LootTableProvider {
    ...
    public static class ModEntityLoot extends EntityLoot {
        ...
        protected void addTables() {
            add(Entities.OWL.get(), LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).add(LootItem.lootTableItem(Items.FEATHER).apply(SetItemCountFunction.setCount(UniformGenerator.between(1.0F, 2.0F))).apply(LootingEnchantFunction.lootingMultiplier(UniformGenerator.between(0.0F, 1.0F))))));
        ...
        protected Iterable<EntityType<?>> getKnownEntities() {
            ...
            for (Map.Entry<ResourceKey<EntityType<?>>, EntityType<?>> entry : ForgeRegistries.ENTITY_TYPES.getEntries()) {
                if (entry.getValue() == Entities.OWL.get()) {
